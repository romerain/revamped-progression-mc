...
import net.rainrome.revampedprogression.recipe.KilnRecipe;
import net.rainrome.revampedprogression.screen.KilnScreenHandler;
import net.rainrome.revampedprogression.util.ModBlockEntityBase;
import org.jetbrains.annotations.Nullable;

import java.util.Optional;

public class KilnBlockEntity extends ModBlockEntityBase {
    ...
    private boolean hasRecipe() {
        for(int i = 0; i < 4; i++) {
            var match = matchRecipe(new SimpleInventory(this.getStack(i)));
            RevampedProgression.LOGGER.info(match.toString());
            if(match.isPresent()) {
                return true;
            }
        }
    ...
    private void produceItem() {
        for(int i = 0; i < 4; i++) {
            var match = matchRecipe(new SimpleInventory(this.getStack(i)));
            if (!this.getStack(i).isEmpty() && match.isPresent()) {
                this.setStack(i, match.get().value().getResult(null));
            }
        }
    }

    private Optional<RecipeEntry<KilnRecipe>> matchRecipe(SimpleInventory match) {
        SimpleInventory inv = new SimpleInventory(match.size());
        for(int i = 0; i < match.size(); i++) {
            inv.setStack(i, match.getStack(i));
        }
